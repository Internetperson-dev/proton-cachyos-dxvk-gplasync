...
|                      | `WINEALSA_CHANNELS`           | Allows to select more than two channels with the winealsa driver as well as force disable spatial audio in games. Can be useful with surround setups or when the sounds come out wrong positionally (like dialogue coming out only from the left speaker). Possible values is the number of speakers, such as `4` (3.1), `6` (5.1) or `8` (7.1) |
