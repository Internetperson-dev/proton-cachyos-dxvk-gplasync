...
| `dxvkllasync`        | `PROTON_DXVK_LLASYNC`          | Enable the alternative DXVK, which contains patches from [dxvk-low-latency](https://github.com/netborg-afps/dxvk/) and [dxvk-gplasync](https://gitlab.com/Ph42oN/dxvk-gplasync/) with default configuration. This means that both `lowlatency` and `gplasync` will be enabled. To enable each component selectively, see below. Async is enabled by default, so it **SHOULD NOT** to be used with games using anti-cheat or multiplayer games in general. You have been warned. |
| `dxvklowlatency`     | `PROTON_DXVK_LOWLATENCY`       | Enable the alternative DXVK mentioned above, with configuration to enable only the `low-latency` component. The configuration is applied by appending to `DXVK_CONFIG` and is equivalent to `DXVK_CONFIG="dxvk.enableAsync=False"`. This option should make it safe to use with anti-cheat/multiplayer games.                                                                                                                                                                   |
| `dxvkgplasync`       | `PROTON_DXVK_GPLASYNC`         | Enable the alternative DXVK mentioned above, with configuration to enable only the `gplasync` component. The configuration is applied by appending to `DXVK_CONFIG` and is equivalent to `DXVK_CONFIG="dxvk.framePace=max-frame-latency"`. This option **SHOULD NOT** be used with anti-cheat/multiplayer games.                                                                                                                                                                |
...
| `nowmdecoration`     | `PROTON_NO_WM_DECORATION`      | Disable window decorations by the the window manager, use wine's own windows decorations.                                                                                                                                                                                                                                                                                                                                                                                       |
