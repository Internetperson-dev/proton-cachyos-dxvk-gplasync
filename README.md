...
| `wayland`            | `PROTON_ENABLE_WAYLAND`        | Enable Wayland support. `winewayland` is experimental and work-in-progress, expect issues. When reporting issues ALWAYS mention if you using this environment variable, and make sure the issue still happens without it. Valve's Protons do not include `winewayland.drv`, they always use `winex11.dr`. For more information refer to [this section](https://github.com/CachyOS/proton-cachyos?tab=readme-ov-file#proton-wayland-quirks)                                         |
...
If something that uses Electron/CEF (launchers like Battle.net, Ubisoft Connect, EA App, Rockstar Launcher) doesn't work after enabling native Wayland support, try adding `--in-process-gpu` to the command arguments. If you encounter controller issues with `winewayland.dr`, try using `PROTON_USE_SDL=1`, or running Steam with `-steamos3` which might allow Steam Input to work. For other options relating to `winewayland.drv` refer to [Proton-EM documentation](https://github.com/Etaash-mathamsetty/Proton/blob/em-10/docs/EM-ADDITIONS.md)
